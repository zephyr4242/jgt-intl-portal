...
<!--
  PortofolioCascaderBase  投组级联单选组件
  不要在这个组件内写业务
 -->
<template>
  <div ref="reference" :class="[
    ...
    <div v-show="isMulti" class="portofolio-cascader-base-input">
      <div class="portofolio-cascader-base-input-text-container">
        <span class="portofolio-cascader-base-input-text">
        已选{{ tips && tips.length }}
       </span>
      </div>
    </div>

    <el-input ref="input" v-model="inputValue" :size="realSize" :placeholder="placeholder" :validate-event="false"
      :class="{ 'is-focus': dropDownVisible,'portofolio-cascader-base-multi':isMulti }" @focus="handleFocus" @blur="handleBlur" @input="handleInput"
      :style="{ width: defaultWidth }" :readonly="loading" :disabled="disabled">
      <template slot="suffix">
        <i v-if="!disableSuffix" key="arrow-down" class="icon jgt-iconfont select-box-icon iconfont-xialafuxuan"
          style="line-height: 32px;cursor: pointer;" @click.stop="suffixClick"></i>
        <i v-else key="arrow-down"
          :class="[
            'el-input__icon',
            'el-icon-arrow-down',
    ...
    <transition name="el-zoom-in-top" @after-leave="handleDropdownLeave">
      <div v-show="dropDownVisible" ref="popper" :class="['el-popper', 'el-cascader__dropdown', popperClass]">
        <PortofolioCascaderPanel ref="panel" v-model="checkedValue" :options="options" :props="config" :border="false"
          :render-label="$scopedSlots.default" @expand-change="handleExpandChange" @close="toggleDropDownVisible(false)"
          class="portofolio-cascader-base-panel" :defaultWidth="defaultWidth" />
      </div>
    </transition>
  </div>
</template>
...
import PortofolioCascaderPanel from './Panel'
...
export default {
  ...
  props: {
    ...
    popperClass: String,
    // 新增，如果传true，会禁用打开弹窗按钮，比如单笔交易等场景
    disableSuffix: {
      type: Boolean,
      default: false
    },
  ...
  mounted() {
    ...
    this.filterHandler = debounce(this.debounce, () => {
      ...
      this.$emit('remoteFilter', inputValue)
      if (!inputValue) {
  ...
  methods: {
    ...
    computePresentText() {
      if (Array.isArray(this.value)) {
        if (this.value[0] === '-1') {
          this.presentText = '全部基煜账户'
          return
        }
        if (this.value[0] === '-2') {
          this.presentText = this.tips?.text
          return
        }
        if (this.value[0] === '-3') {
          this.presentText = '请选择基煜账户'
          return
        }
        if (this.value[0] === '-4') {
          this.presentText = '暂无数据'
          return
        }
      }
      ...
      if (!this.isEmptyValue(checkedValue)) {
        const v = Array.isArray(checkedValue) ? checkedValue[checkedValue.length - 1] : checkedValue
        let flag = false
        // 改动点，直接使用原数据搜索展示文字
        this.options.forEach(item => {
          if (item.value === v) {
            this.presentText = item.label
            flag = true
          }
          if (item?.accountList?.length > 0) {
            item.accountList.forEach(i => {
              if (i.value === v) {
                this.presentText = i.label
                flag = true
              }
            })
          }
        ...
        if (!flag && this.tips?.text) { // options找不到text的时候，使用传入的tips作为显示文字
          this.presentText = this.tips.text
        }
      } else {
...
<style lang="scss">
.portofolio-cascader-base-input {
  position: absolute;
  z-index: 20;
  font-size: 13px;
  border-radius: 2px;
  ...
  pointer-events: none;
  .portofolio-cascader-base-input-text-container{
    display: inline-block;
    width: 72px;
    position: absolute;
    text-align: center;
    .portofolio-cascader-base-input-text{
...
.portofolio-cascader-base-multi .el-input__inner{
  padding-left: 72px; // 显示已选的时候 左侧预留位置
}

.portofolio-cascader-base-panel.el-cascader-panel {
  .infinite-list {
    overflow: auto;
    max-height: 300px;
  }
  ...
  .el-cascader-menu__empty-text.el-cascader-node{
    top: 16px;
  }
...
li.el-cascader__suggestion-item:hover,
li.el-cascader__suggestion-item.is-checked {
  @include color(A10);
}
...
div.portofolio-cascader-base-panel.el-cascader-panel .el-cascader-menu:last-child{
